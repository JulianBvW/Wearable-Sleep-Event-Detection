...
from wearsed.training.batching_plethpre import get_multi_batch, get_test_batch
from wearsed.models.attention_unet.AttentionUNet_plethpre import AttentionUNet
...
parser.add_argument('--use-attention', help='what attention parts should be used [gates,bottleneck,se]', default='', type=str, required=False)
...
# Dataset
full_dataset = WearSEDDataset(signals_to_read=['SpO2'], use_predicted_hypnogram=args.use_predicted_hypnogram, pleth_statistical=True, pleth_vae_latents=True)
train_ids, test_ids = get_fold(FOLD_NAME, args.fold_nr, seed=SEED)

# Model, Optimizer, Criterion
model = AttentionUNet(use_attention=args.use_attention.split(',')).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
criterion = nn.BCEWithLogitsLoss()

show_script_info(args, model, 2+13)
