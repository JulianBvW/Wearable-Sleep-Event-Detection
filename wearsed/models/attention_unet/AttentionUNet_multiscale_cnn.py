...
class AttentionUNet(nn.Module):
    def __init__(self, in_channels=10, out_channels=1, use_attention=['gates', 'bottleneck', 'se']):
        super(AttentionUNet, self).__init__()

        self.use_attention = use_attention

        # PPG festure extraction
        self.ppg_multiscale_cnn = MultiScaleCNN()
    ...
    def feature_extract_ppg(self, x):
        ...
        ppg = self.ppg_multiscale_cnn(ppg)             # [bs, 8, 600]
        if 'se' in self.use_attention:
            ppg = self.se_attention(ppg)               # [bs, 8, 600]

        # Concatinate
        return torch.cat([hyp, spo2, ppg], dim=1)      # [bs, 10, 600]
