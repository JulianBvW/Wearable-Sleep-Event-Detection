...
class WearSEDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        smallest = min([len(s) for s in [hypnogram, spo2]])
        signals = torch.stack([hypnogram[:smallest], spo2[:smallest]])
        ...
        return signals, event_or_not, torch.tensor(pleth, dtype=torch.float32)
