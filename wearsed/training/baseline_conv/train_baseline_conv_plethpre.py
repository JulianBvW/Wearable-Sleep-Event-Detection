'''
Train code for training the Baseline model with preprocessed pleth values
'''
...
parser.add_argument('--use-pleth-statistical', help='use pleth preprocessing: statistical', action='store_true')
parser.add_argument('--use-pleth-vae-latents', help='use pleth preprocessing: vae latents', action='store_true')
...
def get_batch(signals, labels, batch_size, seq_length):
    ...
    for start in random_starts:
        ...
        sequence = signals[:, start:end]  # [num_signals, seq_length]
        batch_signals.append(sequence)
...
# Dataset
full_dataset = WearSEDDataset(signals_to_read=['SpO2', 'Pleth'], pleth_statistical=args.use_pleth_statistical, pleth_vae_latents=args.use_pleth_vae_latents)
train_dataset, test_dataset = torch.utils.data.random_split(full_dataset, [0.8, 0.2])

# Model, Optimizer, Criterion
model = BaselineConv(pleth_statistical=args.use_pleth_statistical, pleth_vae_latents=args.use_pleth_vae_latents).to(device)
