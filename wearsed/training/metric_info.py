...
def get_ahis(y_pred, y_true, hypnogram_data, class_data, thr, correctify=False, correctify_size=10, tsts=None):
    ahis_pred, ahis_true = [], []
    recording_stops = list(y_true[y_true == -999].index)
    for i, (start, end) in enumerate(zip([0] + recording_stops, recording_stops)):
        ahi_pred, ahi_true = get_ahis_single_recording(y_pred[start+1:end], y_true[start+1:end], hypnogram_data[start+1:end], class_data[start+1:end], thr, correctify=correctify, correctify_size=correctify_size, tst=tsts[i] if tsts is not None else None)
        ahis_pred.append(ahi_pred)
        ahis_true.append(ahi_true)
    return ahis_pred, ahis_true

def get_ahis_single_recording(y_pred, y_true, hypnogram_data, class_data, thr, correctify=False, correctify_size=10, tst=None):
    ...
    ahi_true = len(y_true_list) / tst_in_h if tst is None else len(y_true_list) / tst
