...
class BaselineConv(nn.Module):
    ...
    def forward(self, x):                              # [bs, 2+256, 600]
        ...
        # Instead of full hypnogram, only use sleep-or-wake info
        sleep_wake = hyp
        sleep_wake[sleep_wake>1] = 1  # Binary 0 (wake) vs 1 (sleep)
        ...
        # Concatinate
        x = torch.cat([sleep_wake, spo2, pleth], dim=1)  # [bs, 6, 600]
