...
class WearSEDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1 output: One-Hot-Encoded 4-dim Tensor for NOR vs OSA vs CSA vs HYP (1Hz)
        ...
        # Output
        target_OSA = recording.event_df['OSA'] | recording.event_df['MSA']  # Count Mixed Apnea as Obstructive
        target_CSA = recording.event_df['CSA']
        target_HYP = recording.event_df['HYP']
        target_NOR = (target_OSA | target_CSA | target_HYP == 0).astype(int)     # Normal meaning no event
        target = torch.tensor([target_NOR, target_OSA, target_CSA, target_HYP])  # [4, seq_len]
