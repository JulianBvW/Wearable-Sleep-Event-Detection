...
class VAE(nn.Module):
    def __init__(self, input_dim=5*256, latent_dim=8, output_dim=256):
        ...
        # Encoder
        self.enc_conv1 = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=7, stride=5, padding=3)
        self.enc_conv2 = nn.Conv1d(in_channels=16, out_channels=16, kernel_size=5, stride=2, padding=2)
        self.enc_conv3 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=3, stride=2, padding=1)
    ...
    def encode(self, x):                  # [bs,     1280]
        x = x.unsqueeze(1)                # [bs,  1, 1280]
        x = F.relu(self.enc_conv1(x))     # [bs, 16,  256]
        x = F.relu(self.enc_conv2(x))     # [bs, 16,  128]
        x = F.relu(self.enc_conv3(x))     # [bs, 32,   64]
