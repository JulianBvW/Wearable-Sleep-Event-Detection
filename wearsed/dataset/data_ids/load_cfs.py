...
def is_data_available(subject_id):
    ...
    if not os.path.isfile(path_psg) or not os.path.isfile(path_annot):
        return False, False, os.path.isfile(path_scoring)

    # Check SpO2 and PPG signals existing
    edf_reader = pyedflib.EdfReader(path_psg)
    signal_labels = edf_reader.getSignalLabels()

    return True, 'PlethWV' in signal_labels and 'SpO2' in signal_labels, os.path.isfile(path_scoring)
...
available_ids = []
for id in tqdm(ids):
    file_available, ppg_available, scoring_available = is_data_available(id)
    if file_available and ppg_available and scoring_available:
