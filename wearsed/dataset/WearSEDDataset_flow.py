...
class WearSEDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 4 inputs: Hypnogram (1Hz), SpO2 (1Hz), Pleth (256Hz), Flow (32Hz)
        ...
        flow = recording.psg['Flow']
        flow_mean, flow_std, flow_min, flow_max = process_higher_freqs(flow, 32)  # Convert to 4 inputs at 1Hz
        smallest = min([len(s) for s in [hypnogram, spo2, pleth_mean, pleth_std, pleth_min, pleth_max, flow_mean, flow_std, flow_min, flow_max]])
        signals = torch.stack([hypnogram[:smallest], spo2[:smallest], pleth_mean[:smallest], pleth_std[:smallest], pleth_min[:smallest], pleth_max[:smallest], flow_mean[:smallest], flow_std[:smallest], flow_min[:smallest], flow_max[:smallest]])
