...
from wearsed.dataset.WearSEDDataset_flow import WearSEDDataset, get_shakiness
...
# Dataset
full_dataset = WearSEDDataset(signals_to_read=['SpO2', 'Pleth', 'Flow'], preprocess=True)
train_dataset, test_dataset = torch.utils.data.random_split(full_dataset, [0.8, 0.2])

# Model, Optimizer, Criterion
model = BaselineConv(in_channels=10).to(device)
...
for epoch in range(args.epochs):
    ...
    model.train()
    train_loss = 0
    for signals, labels in tqdm(train_dataset):
        optimizer.zero_grad()

        x, y = get_batch(signals, labels, batch_size=args.batch_size, seq_length=args.seq_length)
    ...
    targets = []
    with torch.no_grad():
        for signals, labels in tqdm(test_dataset):
