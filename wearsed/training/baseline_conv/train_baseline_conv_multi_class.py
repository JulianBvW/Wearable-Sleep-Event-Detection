...
from wearsed.models.baseline_conv.BaselineConv_MultiClass import BaselineConv
...
for epoch in range(args.epochs):
    ...
    for i in tqdm(range(len(train_dataset) // args.multi_batch_size)):
        ...
        # Loss computation
        loss = criterion(y_hat, y.argmax(dim=1))
        loss.backward()
    ...
    with torch.no_grad():
        for i in tqdm(range(len(test_dataset) // args.multi_batch_size)):
            ...
            targets.append(y.argmax(dim=1).cpu().flatten())
